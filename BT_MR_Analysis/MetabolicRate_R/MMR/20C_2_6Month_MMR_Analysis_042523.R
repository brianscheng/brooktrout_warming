...
MMR <- read.delim("MMR_2_20C_02142022_raw_adj.txt")
write_csv(MMR,'MMR_2_20C_02212022.csv')
MMR_1_raw_df <- read_delim("MMR_2_20C_02212022.csv", 
  ...
  filter(TimeMinutes <= 180) # Filter to just the first x mins of the df. Using the entire thing makes things run slow later

MMR_1_raw_df$Ch5_DO[is.na(MMR_1_raw_df$Ch5_DO)]=97.04
MMR_1_raw_df$Ch5_temp[is.na(MMR_1_raw_df$Ch5_temp)]=19.78
MMR_1_raw_df <- subset(MMR_1_raw_df, Phase!="Flush")
...
plot_grid(sp1,sp2,sp3,sp4,sp5,sp6,
          labels= "auto",
          align = "h",
          label_size = 14,
          label_colour = "red",
...
ggsave('MMR_all_20C_2_6month_022122.tiff', width = 6, height = 4, units = "in")
...
MMR_1_converted_ch5_df <- data.frame(MMR_1_converted_df$Ch5_DO,
                                     MMR_1_converted_df$Ch5_temp,
                                     MMR_1_converted_df$Ch5_O2mgL,
                                     MMR_1_converted_df$TimeMinutes)

MMR_1_converted_ch5_df <- MMR_1_converted_ch5_df %>%
  rename(Ch5_DO = 1 , Ch5_temp = 2, Ch5_O2mgL = 3, TimeMinutes = 4)
MMR_1_converted_ch5_df <- MMR_1_converted_ch5_df[-c(1:3092),]

BT_5_RR_df_1minute <- as.data.frame(roll_regres(Ch5_O2mgL ~ TimeMinutes, 
                                                MMR_1_converted_ch5_df, 
                                                do_compute = c("sigmas", "r.squareds", "1_step_forecasts"), 
                                                width = 60)) %>% # Set regression window to 1 minute
  filter(!is.na(sigmas)) %>% 
  mutate(RegressionTime = seq(0, by=1/60, length.out=nrow(.)),) %>% 
...
write_csv(mergedBT_RR_df,'_MMR_all_20C_2_6month_042523.csv')
view(mergedBT_RR_df)
