...
ggsave('MMR_all_15C_1_081921.tiff', width = 8, height = 4, units = "in")
...
BT_1_RR_df_2minute <- as.data.frame(roll_regres(Ch1_O2mgL ~ TimeMinutes, 
                                                     MMR_1_converted_df, 
                                                     do_compute = c("sigmas", "r.squareds", "1_step_forecasts"), 
                                                     width = 120)) %>% # Set regression window to 2 minute
  filter(!is.na(sigmas)) %>% 
  mutate(RegressionTime = seq(0, by=1/60, length.out=nrow(.)),) %>% 
  mutate(MMR = ((((vr1 - vf1) * coefs.TimeMinutes)/vf1)* -60)) %>%
  slice(which.max(MMR))
...
write_csv(mergedBT_RR_df,'_MMR_all_15C_1_042523.csv')
view(mergedBT_RR_df)
