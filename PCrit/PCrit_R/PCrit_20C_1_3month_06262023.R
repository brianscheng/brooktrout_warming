...
MR1 = 179.508299541955
MR2 = 143.630691635183
MR3 = 170.289132229616
MR4 = 151.567074879576
MR5 = 178.561649557114
MR6 = 156.20505231535
MR7 = 189.895568961967
MR8 = 169.338428586278

vr1 <- .7268 # Chamber volume in litres
vr2 <- .7268
vr3 <- .7268
vr4 <- .7268
vr5 <- .7268
...
vf1 <- .0174 # Fish volume in litres (1L = 1kg)
vf2 <- .0267
vf3 <- .0323
vf4 <- .0224
vf5 <- .0386
...
PCrit_Raw <- read.delim("PCRIT_1_20C_3month_raw.txt")
write_csv(PCrit_Raw,'PCRIT_1_20C_3month_raw.csv')
PCrit_Raw_modified <- read_delim("PCRIT_1_20C_3month_raw.csv", 
...
PCrit_Raw_df2 <- PCrit_Raw_modified %>% 
  mutate(Ch1_O2mgL = ((respR::convert_DO(x=100, from = "%Air", to = "mg/L", S = Salinity, 
                                         t = Ch1_temp, P = (1.0244))) * (Ch1_DO/100)))%>%
  mutate(Ch2_O2mgL = ((respR::convert_DO(x=100, from = "%Air", to = "mg/L", S = Salinity, 
                                         t = Ch2_temp, P = (1.0244))) * (Ch2_DO/100)))%>%
  mutate(Ch3_O2mgL = ((respR::convert_DO(x=100, from = "%Air", to = "mg/L", S = Salinity, 
                                         t = Ch3_temp, P = (1.0244))) * (Ch3_DO/100)))%>%
  mutate(Ch4_O2mgL = ((respR::convert_DO(x=100, from = "%Air", to = "mg/L", S = Salinity, 
                                         t = Ch4_temp, P = (1.0244))) * (Ch4_DO/100)))%>% 
  mutate(Ch5_O2mgL = ((respR::convert_DO(x=100, from = "%Air", to = "mg/L", S = Salinity, 
                                         t = Ch5_temp, P = (1.0244))) * (Ch5_DO/100))) %>%
  mutate(Ch6_O2mgL = ((respR::convert_DO(x=100, from = "%Air", to = "mg/L", S = Salinity, 
                                         t = Ch6_temp, P = (1.0244))) * (Ch6_DO/100)))%>%
  mutate(Ch7_O2mgL = ((respR::convert_DO(x=100, from = "%Air", to = "mg/L", S = Salinity, 
                                         t = Ch7_temp, P = (1.0244))) * (Ch7_DO/100)))%>%
  mutate(Ch8_O2mgL = ((respR::convert_DO(x=100, from = "%Air", to = "mg/L", S = Salinity, 
                                         t = Ch8_temp, P = (1.0244))) * (Ch8_DO/100))) 
...
Tank7_Pcrit <- data.frame(PCrit_Raw_df2$Ch7_DO,
                          PCrit_Raw_df2$Phase,
                          PCrit_Raw_df2$Salinity,
                          PCrit_Raw_df2$Ch7_temp,
                          PCrit_Raw_df2$patm,
                          PCrit_Raw_df2$Ch7_O2mgL)

Tank8_Pcrit <- data.frame(PCrit_Raw_df2$Ch8_DO,
                          PCrit_Raw_df2$Phase,
                          PCrit_Raw_df2$Salinity,
                          PCrit_Raw_df2$Ch8_temp,
                          PCrit_Raw_df2$patm,
                          PCrit_Raw_df2$Ch8_O2mgL)
...
Tank7_Pcrit$Loop <- 1:nrow(PCrit_Raw_modified)
Tank8_Pcrit$Loop <- 1:nrow(PCrit_Raw_modified)
...
Tank6_Pcrit_converted <- Tank6_Pcrit %>%
  rename(DOpercent = 1 , Phase = 2, Salinity = 3, Temperature = 4, Pressure = 5, O2mgl = 6)
Tank7_Pcrit_converted <- Tank7_Pcrit %>%
  rename(DOpercent = 1 , Phase = 2, Salinity = 3, Temperature = 4, Pressure = 5, O2mgl = 6)
Tank8_Pcrit_converted <- Tank8_Pcrit %>%
  rename(DOpercent = 1 , Phase = 2, Salinity = 3, Temperature = 4, Pressure = 5, O2mgl = 6)
...
Tank7_Pcrit_converted_subset <- subset(Tank7_Pcrit_converted, Phase != "Flush")
Tank7_Pcrit_converted_subset <- subset(Tank7_Pcrit_converted_subset, DOpercent < 90)

x7 <- Tank7_Pcrit_converted_subset$DOpercent
y7 <- 1:length(x7)

Tank7_Pcrit_converted_subset <- data.frame(Tank7_Pcrit_converted_subset %>%
                                             mutate(diffx = x7 - lag(x7, 180), diffy = y7 - lag(y7, 1)))

Tank7_Pcrit_converted_subset <- Tank7_Pcrit_converted_subset[c(1:first(which(Tank7_Pcrit_converted_subset$diffx > 0)))-1,]

Tank7_Pcrit_converted_subset <- subset(Tank7_Pcrit_converted_subset, DOpercent > LOE7)

## Tank 8 Subset ##

Tank8_Pcrit_converted_subset <- subset(Tank8_Pcrit_converted, Phase != "Flush")
Tank8_Pcrit_converted_subset <- subset(Tank8_Pcrit_converted_subset, DOpercent < 90)

x8 <- Tank8_Pcrit_converted_subset$DOpercent
y8 <- 1:length(x8)

Tank8_Pcrit_converted_subset <- data.frame(Tank8_Pcrit_converted_subset %>%
                                             mutate(diffx = x8 - lag(x8, 180), diffy = y8 - lag(y8, 1)))

Tank8_Pcrit_converted_subset <- Tank8_Pcrit_converted_subset[c(1:first(which(Tank8_Pcrit_converted_subset$diffx > 0)))-1,]

Tank8_Pcrit_converted_subset <- subset(Tank8_Pcrit_converted_subset, DOpercent > LOE8)
...
Tank7_Pcrit_converted_subset$Duration <- Tank7_Pcrit_converted_subset$Loop/60
Tank8_Pcrit_converted_subset$Duration <- Tank8_Pcrit_converted_subset$Loop/60
...
Tank7_Pcrit_bins <- make_bins(o2 = Tank7_Pcrit_converted_subset$DOpercent,
                              duration = Tank7_Pcrit_converted_subset$Duration,
                              min_o2_width = 1/100,
                              max_o2_width = 1/5,
                              n_bins = 8)
Tank7_Pcrit_MO2 <- calc_MO2(duration = Tank7_Pcrit_converted_subset$Duration,
                            o2 = Tank7_Pcrit_converted_subset$DOpercent,
                            bin_width = Tank7_Pcrit_bins,
                            vol = vr7,
                            temp = Tank7_Pcrit_converted_subset$Temperature,
                            sal = Tank7_Pcrit_converted_subset$Salinity)
Tank7_Pcrit_MO2_converted <- data.frame(Tank7_Pcrit_MO2 %>%
                                          mutate(conv_o2(o2 = Tank7_Pcrit_MO2$MO2, 
                                                         from = "umol_per_l", 
                                                         to = "mg_per_l", 
                                                         temp = mean(Tank7_Pcrit_converted_subset$Temperature), 
                                                         sal = mean(Tank7_Pcrit_converted_subset$Salinity), 
                                                         atm_pres = mean(Tank7_Pcrit_converted_subset$Pressure))))

Tank7_Pcrit_MO2_converted <- Tank7_Pcrit_MO2_converted %>%
  rename(MO2mgl = 9)
Tank7_Pcrit_MO2_converted <- data.frame(Tank7_Pcrit_MO2_converted %>%
                                          mutate(MO2mgl/vf7))
Tank7_Pcrit_MO2_converted <- Tank7_Pcrit_MO2_converted %>%
  rename(MO2mgkghr = 10)


Tank7_Pcrit_MO2_duration <- data.frame(Tank7_Pcrit_MO2_converted$DUR_RANGE)
Tank7_PCrit_MO2_duration_subset <- Tank7_Pcrit_MO2_duration %>%
  mutate(Tank7_Pcrit_MO2_converted.DUR_RANGE = strsplit(as.character(Tank7_Pcrit_MO2_converted.DUR_RANGE), "-")) %>%
  unnest(cols = Tank7_Pcrit_MO2_converted.DUR_RANGE)
Tank7_PCrit_MO2_duration_start <- Tank7_PCrit_MO2_duration_subset %>%
  filter(row_number() %% 2 == 1) ## Select odd rows
Tank7_PCrit_MO2_duration_start <- data.frame(Tank7_PCrit_MO2_duration_start)
Tank7_Pcrit_MO2_converted <- data.frame(Tank7_Pcrit_MO2_converted %>%
                                          mutate(as.numeric(Tank7_PCrit_MO2_duration_start$Tank7_Pcrit_MO2_converted.DUR_RANGE)))
Tank7_Pcrit_MO2_converted <- Tank7_Pcrit_MO2_converted %>%
  rename(DUR_START = 11)

## TANK8 MO2 Calc ##

Tank8_Pcrit_bins <- make_bins(o2 = Tank8_Pcrit_converted_subset$DOpercent,
                              duration = Tank8_Pcrit_converted_subset$Duration,
                              min_o2_width = 1/100,
                              max_o2_width = 1/5,
                              n_bins = 8)
Tank8_Pcrit_MO2 <- calc_MO2(duration = Tank8_Pcrit_converted_subset$Duration,
                            o2 = Tank8_Pcrit_converted_subset$DOpercent,
                            bin_width = Tank8_Pcrit_bins,
                            vol = vr8,
                            temp = Tank8_Pcrit_converted_subset$Temperature,
                            sal = Tank8_Pcrit_converted_subset$Salinity)
Tank8_Pcrit_MO2_converted <- data.frame(Tank8_Pcrit_MO2 %>%
                                          mutate(conv_o2(o2 = Tank8_Pcrit_MO2$MO2, 
                                                         from = "umol_per_l", 
                                                         to = "mg_per_l", 
                                                         temp = mean(Tank8_Pcrit_converted_subset$Temperature), 
                                                         sal = mean(Tank8_Pcrit_converted_subset$Salinity), 
                                                         atm_pres = mean(Tank8_Pcrit_converted_subset$Pressure))))

Tank8_Pcrit_MO2_converted <- Tank8_Pcrit_MO2_converted %>%
  rename(MO2mgl = 9)
Tank8_Pcrit_MO2_converted <- data.frame(Tank8_Pcrit_MO2_converted %>%
                                          mutate(MO2mgl/vf8))
Tank8_Pcrit_MO2_converted <- Tank8_Pcrit_MO2_converted %>%
  rename(MO2mgkghr = 10)


Tank8_Pcrit_MO2_duration <- data.frame(Tank8_Pcrit_MO2_converted$DUR_RANGE)
Tank8_PCrit_MO2_duration_subset <- Tank8_Pcrit_MO2_duration %>%
  mutate(Tank8_Pcrit_MO2_converted.DUR_RANGE = strsplit(as.character(Tank8_Pcrit_MO2_converted.DUR_RANGE), "-")) %>%
  unnest(cols = Tank8_Pcrit_MO2_converted.DUR_RANGE)
Tank8_PCrit_MO2_duration_start <- Tank8_PCrit_MO2_duration_subset %>%
  filter(row_number() %% 2 == 1) ## Select odd rows
Tank8_PCrit_MO2_duration_start <- data.frame(Tank8_PCrit_MO2_duration_start)
Tank8_Pcrit_MO2_converted <- data.frame(Tank8_Pcrit_MO2_converted %>%
                                          mutate(as.numeric(Tank8_PCrit_MO2_duration_start$Tank8_Pcrit_MO2_converted.DUR_RANGE)))
Tank8_Pcrit_MO2_converted <- Tank8_Pcrit_MO2_converted %>%
  rename(DUR_START = 11)
...
Tank7_Pcrit_MO2_converted_final <- subset(Tank7_Pcrit_MO2_converted, MO2mgkghr > 0)
Tank8_Pcrit_MO2_converted_final <- subset(Tank8_Pcrit_MO2_converted, MO2mgkghr > 0)
...
calc_pcrit(Tank7_Pcrit_MO2_converted_final$O2_MEAN, Tank7_Pcrit_MO2_converted_final$MO2mgkghr, MR = MR7)
calc_pcrit(Tank8_Pcrit_MO2_converted_final$O2_MEAN, Tank8_Pcrit_MO2_converted_final$MO2mgkghr, MR = MR8)
...
write_csv(combined_PCrit_FinalData,'20C_1_PCrit_3month_06262023.csv')

tiff(filename = "Tank1_Final_PCrit_20C_1_06262023.tiff",
     width = 6, height = 6, units = "in", res = 600)
plot_pcrit(Tank1_Pcrit_MO2_converted_final$O2_MEAN, Tank1_Pcrit_MO2_converted_final$MO2mgkghr, MR = MR1)
dev.off()

tiff(filename = "Tank2_Final_PCrit_20C_1_06262023.tiff",
     width = 6, height = 6, units = "in", res = 600)
plot_pcrit(Tank2_Pcrit_MO2_converted_final$O2_MEAN, Tank2_Pcrit_MO2_converted_final$MO2mgkghr, MR = MR2)
dev.off()

tiff(filename = "Tank3_Final_PCrit_20C_1_06262023.tiff",
     width = 6, height = 6, units = "in", res = 600)
plot_pcrit(Tank3_Pcrit_MO2_converted_final$O2_MEAN, Tank3_Pcrit_MO2_converted_final$MO2mgkghr, MR = MR3)
dev.off()

tiff(filename = "Tank4_Final_PCrit_20C_1_06262023.tiff",
     width = 6, height = 6, units = "in", res = 600)
plot_pcrit(Tank4_Pcrit_MO2_converted_final$O2_MEAN, Tank4_Pcrit_MO2_converted_final$MO2mgkghr, MR = MR4)
dev.off()

tiff(filename = "Tank5_Final_PCrit_20C_1_06262023.tiff",
     width = 6, height = 6, units = "in", res = 600)
plot_pcrit(Tank5_Pcrit_MO2_converted_final$O2_MEAN, Tank5_Pcrit_MO2_converted_final$MO2mgkghr, MR = MR5)
dev.off()

tiff(filename = "Tank6_Final_PCrit_20C_1_06262023.tiff",
     width = 6, height = 6, units = "in", res = 600)
plot_pcrit(Tank6_Pcrit_MO2_converted_final$O2_MEAN, Tank6_Pcrit_MO2_converted_final$MO2mgkghr, MR = MR6)
dev.off()

tiff(filename = "Tank7_Final_PCrit_20C_1_06262023.tiff",
     width = 6, height = 6, units = "in", res = 600)
plot_pcrit(Tank7_Pcrit_MO2_converted_final$O2_MEAN, Tank7_Pcrit_MO2_converted_final$MO2mgkghr, MR = MR7)
dev.off()

tiff(filename = "Tank8_Final_PCrit_20C_1_06262023.tiff",
     width = 6, height = 6, units = "in", res = 600)
plot_pcrit(Tank8_Pcrit_MO2_converted_final$O2_MEAN, Tank8_Pcrit_MO2_converted_final$MO2mgkghr, MR = MR8)
dev.off()
...
ggplot(Tank1_Pcrit_converted, aes(x = Loop, y = DOpercent)) + 
  ...
  labs(title = "Pcrit Tank 1 Trial 20C 1", x = "Time", y = "O2 (%)") +
  theme_classic()

ggsave('PCrit_FullRun_Tank1_20C_1_qualitycontrol.tiff')

ggplot(Tank2_Pcrit_converted, aes(x = Loop, y = DOpercent)) + 
  geom_point(na.rm = TRUE) +  ##This means don't plot anything that has an NA 
  labs(title = "Pcrit Tank 2 Trial 20C 1", x = "Time", y = "O2 (%)") +
  theme_classic()

ggsave('PCrit_FullRun_Tank2_20C_1_qualitycontrol.tiff')

ggplot(Tank3_Pcrit_converted, aes(x = Loop, y = DOpercent)) + 
  geom_point(na.rm = TRUE) +  ##This means don't plot anything that has an NA 
  labs(title = "Pcrit Tank 3 Trial 20C 1", x = "Time", y = "O2 (%)") +
  theme_classic()

ggsave('PCrit_FullRun_Tank3_20C_1_qualitycontrol.tiff')

ggplot(Tank4_Pcrit_converted, aes(x = Loop, y = DOpercent)) + 
  geom_point(na.rm = TRUE) +  ##This means don't plot anything that has an NA 
  labs(title = "Pcrit Tank 4 Trial 20C 1", x = "Time", y = "O2 (%)") +
  theme_classic()

ggsave('PCrit_FullRun_Tank4_20C_1_qualitycontrol.tiff')

ggplot(Tank5_Pcrit_converted, aes(x = Loop, y = DOpercent)) + 
  geom_point(na.rm = TRUE) +  ##This means don't plot anything that has an NA 
  labs(title = "Pcrit Tank 5 Trial 20C 1", x = "Time", y = "O2 (%)") +
  theme_classic()

ggsave('PCrit_FullRun_Tank5_20C_1_qualitycontrol.tiff')

ggplot(Tank6_Pcrit_converted, aes(x = Loop, y = DOpercent)) + 
  geom_point(na.rm = TRUE) +  ##This means don't plot anything that has an NA 
  labs(title = "Pcrit Tank 6 Trial 20C 1", x = "Time", y = "O2 (%)") +
  theme_classic()

ggsave('PCrit_FullRun_Tank6_20C_1_qualitycontrol.tiff')

ggplot(Tank7_Pcrit_converted, aes(x = Loop, y = DOpercent)) + 
  geom_point(na.rm = TRUE) +  ##This means don't plot anything that has an NA 
  labs(title = "Pcrit Tank 7 Trial 20C 1", x = "Time", y = "O2 (%)") +
  theme_classic()

ggsave('PCrit_FullRun_Tank7_20C_1_qualitycontrol.tiff')

ggplot(Tank8_Pcrit_converted, aes(x = Loop, y = DOpercent)) + 
  geom_point(na.rm = TRUE) +  ##This means don't plot anything that has an NA 
  labs(title = "Pcrit Tank 8 Trial 20C 1", x = "Time", y = "O2 (%)") +
  theme_classic()

ggsave('PCrit_FullRun_Tank8_20C_1_qualitycontrol.tiff')
...
ggsave('PCrit_FullRun_Tank1_20C_1_postqualitycontrol.tiff')
...
ggsave('PCrit_FullRun_Tank2_20C_1_postqualitycontrol.tiff')
...
ggsave('PCrit_FullRun_Tank3_20C_1_postqualitycontrol.tiff')
...
ggsave('PCrit_FullRun_Tank4_20C_1_postqualitycontrol.tiff')
...
ggsave('PCrit_FullRun_Tank5_20C_1_postqualitycontrol.tiff')
...
ggsave('PCrit_FullRun_Tank6_20C_1_postqualitycontrol.tiff')

ggplot(Tank7_Pcrit_converted_subset, aes(x = Loop, y = DOpercent)) + 
  geom_point(na.rm = TRUE) +  ##This means don't plot anything that has an NA 
  labs(title = "Pcrit Tank 7 Trial 20C 1", x = "Time", y = "O2 (%)") +
  theme_classic()

ggsave('PCrit_FullRun_Tank7_20C_1_postqualitycontrol.tiff')

ggplot(Tank8_Pcrit_converted_subset, aes(x = Loop, y = DOpercent)) + 
  geom_point(na.rm = TRUE) +  ##This means don't plot anything that has an NA 
  labs(title = "Pcrit Tank 8 Trial 20C 1", x = "Time", y = "O2 (%)") +
  theme_classic()

ggsave('PCrit_FullRun_Tank8_20C_1_postqualitycontrol.tiff')

## This plots MO2 vs O2 ##

ggplot(Tank1_Pcrit_MO2_converted_final, aes(x = O2_MEAN, y = MO2mgkghr)) + 
  geom_point(na.rm = TRUE) +  ##This means don't plot anything that has an NA 
  labs(title = "Pcrit Tank 1 Trial 20C 1", x = "O2 (%)", y = "MO2") +
  theme_classic()

ggsave('PCrit_Tank1_20C_1_06262023.tiff')

ggplot(Tank2_Pcrit_MO2_converted_final, aes(x = O2_MEAN, y = MO2mgkghr)) + 
  geom_point(na.rm = TRUE) +  ##This means don't plot anything that has an NA 
  labs(title = "Pcrit Tank 2 Trial 20C 1", x = "O2 (%)", y = "MO2") +
  theme_classic()

ggsave('PCrit_Tank2_20C_1_06262023.tiff')

ggplot(Tank3_Pcrit_MO2_converted_final, aes(x = O2_MEAN, y = MO2mgkghr)) + 
  geom_point(na.rm = TRUE) +  ##This means don't plot anything that has an NA 
  labs(title = "Pcrit Tank 3 Trial 20C 1", x = "O2 (%)", y = "MO2") +
  theme_classic()

ggsave('PCrit_Tank3_20C_1_06262023.tiff')

ggplot(Tank4_Pcrit_MO2_converted_final, aes(x = O2_MEAN, y = MO2mgkghr)) + 
  geom_point(na.rm = TRUE) +  ##This means don't plot anything that has an NA 
  labs(title = "Pcrit Tank 4 Trial 20C 1", x = "O2 (%)", y = "MO2") +
  theme_classic()

ggsave('PCrit_Tank4_20C_1_06262023.tiff')

ggplot(Tank5_Pcrit_MO2_converted_final, aes(x = O2_MEAN, y = MO2mgkghr)) + 
  geom_point(na.rm = TRUE) +  ##This means don't plot anything that has an NA 
  labs(title = "Pcrit Tank 5 Trial 20C 1", x = "O2 (%)", y = "MO2") +
  theme_classic()

ggsave('PCrit_Tank5_20C_1_06262023.tiff')

ggplot(Tank6_Pcrit_MO2_converted_final, aes(x = O2_MEAN, y = MO2mgkghr)) + 
  geom_point(na.rm = TRUE) +  ##This means don't plot anything that has an NA 
  labs(title = "Pcrit Tank 6 Trial 20C 1", x = "O2 (%)", y = "MO2") +
  theme_classic()

ggsave('PCrit_Tank6_20C_1_06262023.tiff')

ggplot(Tank7_Pcrit_MO2_converted_final, aes(x = O2_MEAN, y = MO2mgkghr)) + 
  geom_point(na.rm = TRUE) +  ##This means don't plot anything that has an NA 
  labs(title = "Pcrit Tank 7 Trial 20C 1", x = "O2 (%)", y = "MO2") +
  theme_classic()

ggsave('PCrit_Tank7_20C_1_06262023.tiff')

ggplot(Tank8_Pcrit_MO2_converted_final, aes(x = O2_MEAN, y = MO2mgkghr)) + 
  geom_point(na.rm = TRUE) +  ##This means don't plot anything that has an NA 
  labs(title = "Pcrit Tank 8 Trial 20C 1", x = "O2 (%)", y = "MO2") +
  theme_classic()

ggsave('PCrit_Tank8_20C_1_06262023.tiff')
